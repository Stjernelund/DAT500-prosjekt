...
if __name__ == "__main__":
    ...
    df1 = df1.withColumn(
        "paper_id", f.split(f.col("value"), "\\t").getItem(0)
    ).withColumn("text", f.split(f.col("value"), "\\t").getItem(1))
    df1 = (
        df1.select(f.split(df1.value, "\\t"))
        .rdd.flatMap(lambda x: x)
        .toDF(schema=["paper_id", "text"])
    )
    ...
    # vectorize
    vectorizer = CountVectorizer(inputCol="words", outputCol="vectorizer").fit(
        wordsData
    )
    ...
    # create dense vector
    outputData = wordsData.withColumn(
        "tfidf_features_dense", to_dense_udf("tfidf_features")
    )
    outputData.show()
    spark.stop()
